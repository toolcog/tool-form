...
export function processNode(
  ...
  if (node === undefined) {
    ...
    return processDirectives(node, context);
  }
  return node;
}
...
export function processArray(
  ...
  for (let index = 0; index < node.length; index += 1) {
    ...
    nestFrame(context, (frame: Frame): void => {
      ...
      const transformed = processNode(item, context);
...
export function processObject(
  ...
  for (let [key, value] of Object.entries(node)) {
    ...
    nestFrame(context, (frame: Frame): void => {
      ...
      const transformed = processNode(value, context);
      if (transformed === undefined) {
        modified = true;
...
export function processDirectives(
  ...
  if (directives.modifiers !== undefined) {
    for (const directive of Object.values(directives.modifiers)) {
      directive.modify(node, context);
    }
  }

  // Apply the domain directive, or transform the object.
  if (directives.domain !== undefined) {
    result = directives.domain.transform(node, context);
  } else {
    result = processObject(node, context);
  }
  ...
  if (directives.operators !== undefined) {
    for (const [directive, argument] of Object.values(directives.operators)) {
      result = directive.operate(argument, result, context);
    }
  }
